    ...
    public class TicketService : ITicketService
    {
        ...
        public int? CreateOrUpdateTicket(Ticket ticket)
        {
            if (ticket.TicketId == 0) //new Ticket
            {
                ...
                ticket.CreatedBy = Security.CurrentUserName;
                ...
                rnv.Add(ValidateTicketDetailFields(ticket));// enforce field rules
                ...
                AddAttachmentsToNewTicket(ticket);
                //comment
                TicketComment openingComment = (!ticket.Owner.Equals(Security.CurrentUserName, StringComparison.InvariantCultureIgnoreCase)) ?
                    GetActivityComment(TicketActivity.CreateOnBehalfOf, TicketCommentFlag.CommentNotApplicable, null, ticket.AssignedTo, ticket.GetNotificationSubscribers(), Security.GetUserDisplayName(ticket.Owner)) :
                    GetActivityComment(TicketActivity.Create, TicketCommentFlag.CommentNotApplicable, null, ticket.AssignedTo, ticket.GetNotificationSubscribers());

                ticket.TicketComments.Add(openingComment);

                int? newTicketId = null;
                if (Repository.CreateTicket(ticket, true))
                {
            ...
            {
                UpdateTicket(ticket, string.Empty);
                return ticket.TicketId;
            }
            return null;
        }

        public bool UpdateTicket(Ticket newticket, string comment)
        {
            // do some stupid work here to overcome EntityKey

            Ticket ticket = GetTicket(newticket.TicketId);
            ticket.Title = newticket.Title;
            ticket.Type = newticket.Type;
            ticket.Result = newticket.Result;
            ticket.Priority = newticket.Priority;
            ticket.Category = newticket.Category;
            ticket.SubCategory = newticket.SubCategory;
            ticket.TagList = newticket.TagList;
            ticket.Owner = newticket.Owner;
            ticket.Details = newticket.Details;
            ticket.TestOutline = newticket.TestOutline;
            ticket.TargetDevice = newticket.TargetDevice;
            ticket.TargetVersion = newticket.TargetVersion;
            ticket.AnalysisDetail = newticket.AnalysisDetail;
            ticket.AnalysisRemark = newticket.AnalysisRemark;
            ticket.AssignedTo = newticket.AssignedTo;
            ticket.CurrentStatus = newticket.CurrentStatus;
            ticket.AnalysisNote = newticket.AnalysisNote;
            //ticket.TicketAttachments = newticket.TicketAttachments;
